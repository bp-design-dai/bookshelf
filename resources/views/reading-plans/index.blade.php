                    ...
                    >
                        ...
                        @foreach (\App\Enums\ReadingPlanStatus::cases() as $statusOption)
                            <option
                                ...
                                @selected($currentStatus === $statusOption->value)
                            >
                                {{ $statusOption->label() }}
                            </option>
                        @endforeach
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    @else
                                ...
                                @foreach ($readingPlans as $plan)
                                    @php
                                        $statusClass = match ($plan->status) {
                                            \App\Enums\ReadingPlanStatus::InProgress => 'bg-blue-100 text-blue-800',
                                            \App\Enums\ReadingPlanStatus::Completed => 'bg-green-100 text-green-800',
                                            \App\Enums\ReadingPlanStatus::Expired => 'bg-red-100 text-red-800',
                                        };
                                    @endphp
                                    ...
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <a
                                                href="{{ route('books.show', $plan->book) }}"
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                                            @if ($plan->status === \App\Enums\ReadingPlanStatus::InProgress)
                                                <form
                                                    action="{{ route('reading-plans.complete', $plan) }}"
                                                    method="POST"
                                                    class="inline"
                                                    novalidate
                                                >
                                                ...
                                                <a
                                                    href="{{ route('reading-plans.edit', $plan) }}"
                        ...
                        <div class="mt-6">
                            {{ $readingPlans->withQueryString()->links() }}
                        </div>
                    @endif
                </div>
            </div>
